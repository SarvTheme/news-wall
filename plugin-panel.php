    ...
    <div class="nw_setting">
        <form method="POST" class="nw_setting_form">
            ...
            if(isset($_POST['nw_option_submit'])){
                ...
                $table_name = $wpdb->prefix . NW_OPTIONS_TABLE_NAME;
                if (nwap_check_input($nw_newsnum) AND nwap_check_input($nw_btntitle) AND $nw_sectitle) {
                    $nw_newsnum = (int) $nw_newsnum;
                    if ($wpdb->update($table_name, ['value' => $nw_newsnum] , ['id' => '2']) OR $wpdb->update($table_name, ['strvalue' => $nw_btntitle] , ['id' => '3']) OR $wpdb->update($table_name, ['strvalue' => $nw_sectitle] , ['id' => '4'])) {
